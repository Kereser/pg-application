package co.com.crediya.application.sqs.listener.helper;

import java.util.concurrent.ExecutorService;
import java.util.concurrent.Executors;
import java.util.function.Function;

import co.com.crediya.application.sqs.listener.config.SQSProperties;
...
public class SQSListener {
  ...
  private static final String LISTEN_RETRY_ERROR = "Error listening sqs queue";
  private static final String DEBUG_GET_MSGS = "{} received messages from sqs";
  ...
  private Flux<Void> listenRetryRepeat() {
    return listen().doOnError(e -> log.error(LISTEN_RETRY_ERROR, e)).repeat();
  }
  ...
  private Flux<Void> listen() {
    return getMessages()
        ...
        .onErrorContinue((e, o) -> log.error(LISTEN_RETRY_ERROR, e));
  }
  ...
  private Flux<Message> getMessages() {
    return Mono.fromCallable(this::getReceiveMessageRequest)
        ...
        .doOnNext(response -> log.debug(DEBUG_GET_MSGS, response.messages().size()))
